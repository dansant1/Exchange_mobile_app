...
Template.registro.onRendered(function () {
    ...
    Template.instance().subirDNI = function  (event, template, id) {

    let archivo12 = document.getElementById("dni2");

    console.log(archivo12);
          ...
          for (var i = 0; i < archivo.files.length; i++) {
            ...
            Fotos.insert(doc, function (err, fileObj) {
                if (err) {
                    ...
                    //document.getElementById('id').innerHTML = 'Imagen Cargada';
...
Template.registro.events({
	'click .r': function (e, t) {
		...
		datos.profile = {
			...
			telefono: t.find("[name='area']").value + t.find("[name='telefono']").value,
			pais: $( "#pais option:selected" ).text(),
			direccion: t.find("[name='direccion']").value,
			ciudad: t.find("[name='ciudad']").value,
      verificado: false
		}

		 if (datos.profile.nombre !== "" && datos.profile.ap !== "" && datos.profile.am !== "" && datos.profile.telefono !== "" && datos.profile.direccion !== "" && datos.profile.ciudad !== "") {

              Meteor.call('agregarUsuario', datos, function (err, result) {
                if (err) {
                  alert(err);
                } else {
                  Meteor.loginWithPassword(datos.email, datos.password);
                  //t.subirDNI(e, t, result.id);
                    let archivo = document.getElementById('dni2')
    ...
    'change #dni2': function () {
        document.getElementById('cargado').innerHTML = 'Imagen Cargada';
    }
});
...
Template.login.events({
	'click .r': function (event, template) {
...
Template.login2.onCreated( function () {
    var self = this;
...
Template.login3.onCreated( function () {
    var self = this;
...
Template.olvide.onCreated( function () {
    var self = this;
...
Template.olvide.events({
    'click .rec': function (e, t) {
        e.preventDefault();
        let options = {
           email: t.find("[name='email']").value
        }
        ...
        if (email !== "") {
            Accounts.forgotPassword(options, function (err) {
                if (err) {
                    ...
                    alert('Te hemos enviado un mail de verificación');
                }
            })
        }
    }
